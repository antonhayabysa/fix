...
const SignUpScreen = ({navigation}) => {
  const [ErrorMessageEmail, setErrorMessageEmail] = React.useState(null);
  const [ErrorMessagePassword, setErrorMessagePassword] = React.useState(null);
  const [ErrorMessageUsername, setErrorMessageUsername] = React.useState(null);
  ...
  const loginHandler = (username, usermail, password) => {
    // dataInput = {
    //     username,
    //     usermail,
    //     password
    ...
    fetch('https://www.ecomap.online/auth/users/', {
      method: 'POST',
      headers: {
      ...
      .then(json => {
        ...
        setErrorMessageEmail(json.email);
        setErrorMessagePassword(json.password);
        setErrorMessageUsername(json.username);
        if (json.email == usermail) {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.form}>
        ...
        {ErrorMessageUsername ? (
          <Text style={styles.textError}>{ErrorMessageUsername}</Text>
        ) : null}
        ...
        <TextInput
          ...
          onChangeText={val => TextMailChange(val)}
        />
        {ErrorMessageEmail ? (
          <Text style={styles.textError}>{ErrorMessageEmail}</Text>
        ) : null}
        <TextInput
        ...
        {ErrorMessagePassword ? (
          <Text style={styles.textError}>{ErrorMessagePassword}</Text>
        ) : null}
        <Pressable
          onPress={() => {
            loginHandler(data.username, data.usermail, data.password);
          }}
          style={({pressed}) => [
            styles.signin,
            {
      ...
      <View style={styles.authsocial}>
        <Text style={styles.authtext}>Регистрация с помощью:</Text>
        <View style={styles.authbuttons}>
          <Pressable
            ...
            onPress={()=>{}}
            >
              <FontAwesomeIcon color='#000' size={42} icon={['fab', 'apple']} />
