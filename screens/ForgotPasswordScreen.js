...
const ForgotPasswordScreen = observer(({navigation}) => {
 const [display_confirmpw, setDisplayConfirmPw]= useState(false)
 
 const [ErrorMessageFieldError, setErrorMessageFieldError] = useState(null);
  const [ErrorMessageDescription, setErrorMessageDescription] = useState(null);
  const [ErrorMessageNewPassword, setErrorMessageNewPassword] = useState(null);
  const [ErrorMessageReNewPassword, setErrorMessageReNewPassword] = useState(null);
 ...
 const handleConfirmpwChange = val => {
  ...
  const handleUpdatePasswordChange =( email,code,password,confirmpw) =>{
   
    fetch('https://www.ecomap.online/auth/users/reset_password_confirm/',{
      method: 'POST',
      headers: {
      ...
      .then(data =>{
        ...
        if(data.status == 204){
          ...
          setErrorMessageFieldError("Проверьте данные в полях")
        }
      })
  }
  useEffect(() => {
  ...
  const resetPasswordHandler = (email) => {
    fetch('https://www.ecomap.online/api/auth/reset-password/',{
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      ...
      .then(data =>{
        ...
        if(data.status == 202){
          setDisplayConfirmPw(true)
              Alert.alert(
                "Success!",
                 `We send to your email: ${email}, the confirmation code`
              );
            }else{
          setErrorMessageDescription("User does not exist!")
        }
  ...
  return (
    ...
    <Text style={styles.logintitle}>Сменить пароль</Text>
    {display_confirmpw? (
...
<TextInput
 ...
 <TextInput
   style={styles.passwordinput}
   placeholder="Подтверждение нового пароля"s
   secureTextEntry={true}
   onChangeText={val => handleConfirmpwChange(val)}
 />
 {ErrorMessageFieldError ? (
 ...
 <Pressable
 onPress={ () => {
  handleUpdatePasswordChange(data.email,code.code, password.password, confirmpw.confirmpw );
}}
 style={({pressed}) => [
   styles.signin,
   {
   ...
   <View>
     {pressed ? (
       <LinearGradient
         ...
         <Text style={styles.signinText}>Изменить</Text>
       </LinearGradient>
     ) : (
       <LinearGradient
    ...
    (
      ...
      <Pressable
        onPress={() => {
          resetPasswordHandler(data.email);
        }}
        style={({pressed}) => [
          styles.signin,
          {
